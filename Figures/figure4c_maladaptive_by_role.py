#!/usr/bin/env python3
"""
Figure 4C: Maladaptive Behavior Emergence Analysis - Split by Role (NB vs DB vs Random)
Analyzes maladaptive behaviors across adversity conditions by role
...
# Maladaptive behavior variable mappings from simulation.py
MALADAPTIVE_BEHAVIORS = {
    ...
    'cynical': 'Negative Expectancy Bias'
}

ADAPTIVE_BEHAVIOR_COLUMNS = [
    'protective_behavior',
...
TOTAL_BEHAVIOR_COLUMNS = list(dict.fromkeys(list(MALADAPTIVE_BEHAVIORS.keys()) + ADAPTIVE_BEHAVIOR_COLUMNS))
...
def extract_maladaptive_behaviors(csv_files):
    """
    Extract maladaptive behavior data from CSV files (NB, DB, and Random)
    Returns DataFrame with run-level maladaptive behavior percentages
    """
    all_data = []
    ...
    for csv_file in csv_files:
        ...
        try:
            ...
            # Process each maladaptive behavior
            for behavior_var, behavior_name in MALADAPTIVE_BEHAVIORS.items():
                if behavior_var not in df_runs.columns:
                    print(f"Warning: {behavior_var} not found in {csv_file}")
                    continue

                if behavior_var == 'cynical':
                    # Negative expectancy includes cynical + learned helplessness.
                    df_runs[behavior_var] = df_runs['cynical'] + df_runs['learned_helplessness']
...
def perform_statistical_tests(df_combined):
    """
    Perform Kruskal-Wallis and Mann-Whitney U tests for maladaptive behaviors by role.
...
def create_visualization(df_combined, output_dir):
    """Create Figure 4C with 4B-style layout and random dashed baselines."""
    os.makedirs(output_dir, exist_ok=True)
    
    behaviors = list(MALADAPTIVE_BEHAVIORS.values())
    ...
    for idx, behavior_name in enumerate(behaviors):
        ax = axes[idx]
        behavior_var = [k for k, v in MALADAPTIVE_BEHAVIORS.items() if v == behavior_name][0]
        pct_col = f'{behavior_var}_pct'
    ...
    plt.tight_layout()
    output_path = os.path.join(output_dir, 'figure4c_maladaptive_by_role.png')
...
def print_summary_table(stats_df):
    ...
    print("\n" + "="*160)
    print("MALADAPTIVE BEHAVIOR BY ROLE ANALYSIS - Summary Statistics")
    print("="*160)
...
def main():
    parser = argparse.ArgumentParser(description='Figure 4C: Maladaptive Behavior by Role Analysis')
    ...
    # Extract data
    df_combined = extract_maladaptive_behaviors(all_files)
    ...
    raw_path = os.path.join(args.output, 'figure4c_raw_behavior_percentages.csv')
    raw_df.to_csv(raw_path, index=False)
    print(f"\nExported raw data to {raw_path}")
    
    # Export statistics
    stats_path = os.path.join(args.output, 'figure4c_stats_kruskal.csv')
