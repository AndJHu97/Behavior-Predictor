#!/usr/bin/env python3
"""
Figure 5: Probability of Emergence of Maladaptive Behaviors
...
EMERGENCE_BEHAVIOR_SPECS = [
    ("bully_behavior", "Misdirected Aggression", "bully_behavior", "total_complex_behaviors"),
    ("aggressive_withdrawn_relationship", "Relational Aggression", "aggressive_withdrawn_relationship", "total_complex_behaviors"),
    ("dangerous_trust", "Dangerous Trust", "dangerous_trust", "total_complex_behaviors"),
    ("fearful_withdrawn_relationship", "Hypervigilant Withdrawal", "fearful_withdrawn_relationship", "total_complex_behaviors"),
    ("negative_expectancy_combo", "Negative Expectancy", "negative_expectancy_combo", "total_expectancies"),
]
...
def run_pipeline(files: List[str], threshold_pct: float) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    runs_df["total_expectancies"] = runs_df[["hopefulness", "cynical", "learned_helplessness"]].sum(axis=1)
    runs_df["negative_expectancy_combo"] = runs_df["cynical"] + runs_df["learned_helplessness"]
    runs_df["positive_expectancy"] = runs_df["hopefulness"]

    # Percent per behavior per run using behavior-appropriate denominators.
    for behavior_key, _label, numerator_col, denom_col in EMERGENCE_BEHAVIOR_SPECS:
    ...
    th_rows = []
    thresholds = {}
    for b, label, _numerator_col, _denom_col in EMERGENCE_BEHAVIOR_SPECS:
    ...
    # Presence labels.
    for b, _label, _numerator_col, _denom_col in EMERGENCE_BEHAVIOR_SPECS:
        t = thresholds[b]
    ...
    for _, r in runs_df.iterrows():
        for b, label, numerator_col, denom_col in EMERGENCE_BEHAVIOR_SPECS:
            raw_long.append({
...
def plot_probability(prob_df: pd.DataFrame, out_png: str) -> None:
    ...
    behaviors = [label for _, label, _, _ in EMERGENCE_BEHAVIOR_SPECS]
    conditions = CONDITIONS
    x = np.arange(len(conditions), dtype=float)
    width = 0.12
    offsets = (np.arange(len(behaviors)) - (len(behaviors) - 1) / 2.0) * width
    ...
    ax.set_title("Figure 5: Likelihood of maladaptive behavior emergence by adversity", fontsize=13, fontweight="bold")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Figure 5: Probability of Emergence of Maladaptive Behaviors")
    ...
    png_path = os.path.join(args.output_dir, "figure5_maladaptive_emergence_probability.png")
    raw_path = os.path.join(args.output_dir, "figure5_raw_run_presence.csv")
    thresholds_path = os.path.join(args.output_dir, "figure5_thresholds.csv")
    prob_path = os.path.join(args.output_dir, "figure5_probability_by_condition.csv")
    stats_path = os.path.join(args.output_dir, "figure5_stats_chi2.csv")
