#!/usr/bin/env python3
"""
Figure 5C: Maladaptive Behavior Emergence Analysis - Split by Role (NB vs DB vs Random)
...
def extract_emergence_data(csv_files: List[str]) -> pd.DataFrame:
    ...
    for csv_file in csv_files:
        ...
        try:
            ...
            df_runs['negative_expectancy_combo'] = df_runs['cynical'] + df_runs['learned_helplessness']

            for behavior_key, behavior_name, numerator_col, denom_col in MALADAPTIVE_EMERGENCE_SPECS:
...
def apply_global_thresholds(df_combined: pd.DataFrame, threshold_pct: float) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for behavior_key, behavior_name, _numerator_col, _denom_col in MALADAPTIVE_EMERGENCE_SPECS:
...
def create_visualization(df_combined: pd.DataFrame, output_dir: str) -> None:
    ...
    behaviors = [label for _, label, _, _ in MALADAPTIVE_EMERGENCE_SPECS]
    ...
    for idx, behavior_name in enumerate(behaviors):
        ax = axes[idx]
        behavior_var = [k for k, v, _, _ in MALADAPTIVE_EMERGENCE_SPECS if v == behavior_name][0]
    ...
    fig.delaxes(axes[5])
    plt.tight_layout()
    output_path = os.path.join(output_dir, 'figure5c_maladaptive_by_role.png')
...
def print_summary_table(stats_df: pd.DataFrame) -> None:
    print('\n' + '=' * 160)
    print('FIGURE 5C: MALADAPTIVE EMERGENCE BY ROLE - Summary Statistics')
    print('=' * 160)
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Figure 5C: Maladaptive Emergence by Role Analysis')
    ...
    raw_saved = write_csv_with_fallback(raw_df, os.path.join(args.output, 'figure5c_raw_emergence_presence.csv'))
    stats_saved = write_csv_with_fallback(stats_df, os.path.join(args.output, 'figure5c_stats_chi2.csv'))
    thresholds_saved = write_csv_with_fallback(thresholds_df, os.path.join(args.output, 'figure5c_thresholds.csv'))
    ...
    prob_saved = write_csv_with_fallback(prob_df, os.path.join(args.output, 'figure5c_probability_by_condition_role.csv'))
