#!/usr/bin/env python3
"""
Figure 7C: Maladaptive behavior emergence by resilient/vulnerable groups.
...
- Misdirected aggression: bully_behavior / total_complex_behaviors
- Relational aggression: aggressive_withdrawn_relationship / total_complex_behaviors
- Dangerous trust: dangerous_trust / total_complex_behaviors
- Negative expectancy: (cynical + learned_helplessness) /
    (hopefulness + cynical + learned_helplessness)

Analysis:
1) Primary multivariate test: MANOVA on the 3 maladaptive complex-behavior percentages.
2) Follow-up individual tests by behavior with multiple-comparison correction
   (Bonferroni and FDR-BH).
"""
...
MALADAPTIVE_COMPLEX_SPECS = [
    ("bully_behavior_pct", "Misdirected aggression", "bully_behavior"),
    (
        "aggressive_withdrawn_relationship_pct",
        "Relational aggression",
        "aggressive_withdrawn_relationship",
    ),
    ("dangerous_trust_pct", "Dangerous trust", "dangerous_trust"),
]

NEGATIVE_EXPECTANCY_SPEC = (
    "negative_expectancy_pct",
    "Negative expectancy",
)
...
def parse_run_rows(csv_path: str) -> List[Dict[str, float]]:
    ...
    for row in rows[header_idx + 1 :]:
        ...
        for pct_col, _label, numerator_col in MALADAPTIVE_COMPLEX_SPECS:
            row_out[pct_col] = (values[numerator_col] / total_complex) * 100.0

        row_out["learned_helplessness_pct"] = (values["learned_helplessness"] / total_complex) * 100.0

        negative_expectancy_numerator = values["cynical"] + values["learned_helplessness"]
        row_out["negative_expectancy_pct"] = (negative_expectancy_numerator / total_expectancy) * 100.0
...
def run_manova(df: pd.DataFrame) -> Tuple[str, Dict[str, float]]:
    ...
    dep_cols = [x[0] for x in MALADAPTIVE_COMPLEX_SPECS]
    formula = f"{' + '.join(dep_cols)} ~ group_code"
    ...
    try:
        ...
        # Attempt to extract Wilks p-value if possible.
        wilks_lambda = np.nan
        f_value = np.nan
        p_value = np.nan
        try:
...
def plot_violin(df: pd.DataFrame, out_png: str) -> None:
    behavior_plot_specs = [
        ("bully_behavior_pct", "Misdirected\nAggression"),
        ("aggressive_withdrawn_relationship_pct", "Relational\nAggression"),
        ("dangerous_trust_pct", "Dangerous\nTrust"),
        ("negative_expectancy_pct", "Negative\nExpectancy"),
    ]
    ...
    ax.set_title("Figure 7C: Maladaptive behavior emergence by resilient-vulnerable groups")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Figure 7C maladaptive emergence by subgroup")
    parser.add_argument(
        "--inputs",
        nargs="*",
    ...
    followup_rows = []
    for col, label, _src in MALADAPTIVE_COMPLEX_SPECS:
        res_vals = raw_df[raw_df["latent_group"] == "resilient"][col].to_numpy(dtype=float)
        vul_vals = raw_df[raw_df["latent_group"] == "vulnerable"][col].to_numpy(dtype=float)
    ...
    neg_col, neg_label = NEGATIVE_EXPECTANCY_SPEC
    res_vals = raw_df[raw_df["latent_group"] == "resilient"][neg_col].to_numpy(dtype=float)
    vul_vals = raw_df[raw_df["latent_group"] == "vulnerable"][neg_col].to_numpy(dtype=float)
    neg_stats = compare_groups(res_vals, vul_vals)
    followup_rows.append(
        {
            "behavior_key": neg_col,
            "behavior": neg_label,
            "denominator": "hopefulness + cynical + learned_helplessness",
            "n_resilient": len(res_vals),
            "n_vulnerable": len(vul_vals),
            **neg_stats,
        }
    ...
    bonf = [np.nan] * len(pvals)
    fdr = [np.nan] * len(pvals)
    if valid_p:
        bonf_adj = holm_adjust(valid_p)
        fdr_adj = fdr_bh_adjust(valid_p)
        for j, i in enumerate(valid_idx):
            bonf[i] = bonf_adj[j]
            fdr[i] = fdr_adj[j]

    followup_df["p_holm_bonferroni"] = bonf
    followup_df["p_fdr_bh"] = fdr
    ...
    desc_rows = []
    for key, label, _src in MALADAPTIVE_COMPLEX_SPECS:
        for grp in ["resilient", "vulnerable"]:
            vals = raw_df[raw_df["latent_group"] == grp][key].to_numpy(dtype=float)
            desc_rows.append(
                {
                    "behavior_key": key,
                    "behavior": label,
    ...
    neg_key, neg_label = NEGATIVE_EXPECTANCY_SPEC
    for grp in ["resilient", "vulnerable"]:
        vals = raw_df[raw_df["latent_group"] == grp][neg_key].to_numpy(dtype=float)
        desc_rows.append(
            {
                "behavior_key": neg_key,
                "behavior": neg_label,
                "denominator": "hopefulness + cynical + learned_helplessness",
                "group": grp,
                "n": len(vals),
    ...
    fig_path = os.path.join(args.output_dir, "figure7C_maladaptive_emergence_violin.png")
    raw_path = os.path.join(args.output_dir, "figure7C_raw_run_percentages.csv")
    desc_path = os.path.join(args.output_dir, "figure7C_descriptive_stats.csv")
    followup_path = os.path.join(args.output_dir, "figure7C_followup_tests.csv")
    manova_txt_path = os.path.join(args.output_dir, "figure7C_manova_results.txt")
    manova_metrics_path = os.path.join(args.output_dir, "figure7C_manova_metrics.csv")
    skipped_path = os.path.join(args.output_dir, "figure7C_skipped_files.csv")
    summary_path = os.path.join(args.output_dir, "figure7C_analysis_summary.txt")
    ...
    with open(manova_txt_path, "w", encoding="utf-8") as f:
        f.write("MANOVA on maladaptive complex-behavior percentages\n")
        f.write("Dependent variables: bully_behavior_pct, aggressive_withdrawn_relationship_pct, dangerous_trust_pct\n")
        f.write("Predictor: latent_group (resilient/vulnerable)\n\n")
        f.write(manova_text)
        f.write("\n")

    summary_lines = [
        "Figure 7C maladaptive emergence by group summary",
        "=" * 72,
        ...
        "",
        "Primary test (MANOVA) on 3 maladaptive complex behaviors:",
